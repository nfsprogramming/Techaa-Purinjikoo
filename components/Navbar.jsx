...
export default function Navbar() {
  ...
  const { xp, level, streak, XP_LEVELS, syncNow, isSyncing } = useUserProgress();
              ...
              <>
                ...
                <button
                  ...
                  style={{
                    ...
                    display: "flex",
                    alignItems: "center",
                    color: isSyncing ? "#8b5cf6" : "rgba(255,255,255,0.5)",
                    fontSize: "0.8rem",
      ...
      {open && (
        <div style={{ background: "rgba(7, 7, 17, 0.98)", padding: "20px 24px", display: "flex", flexDirection: "column", gap: "16px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          {user && (
            ...
            >
              <span>🔄</span> Force Sync Cloud Progress
            </button>
          )}
          {MOBILE_LINKS.map(l => (
          ...
          {user && (
            <button onClick={logout} style={{ color: "#ef4444", fontWeight: 700, background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: "8px 0" }}>Logout</button>
          )}
        </div>
      )}
    </nav>
