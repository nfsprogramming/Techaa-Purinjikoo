...
export default function ProfilePage() {
  const { xp, level, streak, completedTopics, unlockedBadges, XP_LEVELS, BADGES, getLevelProgress, syncNow, isSyncing, syncStatusMessage } = useUserProgress();
  const { user } = useAuth();
  ...
  return (
    <div style={{ background: "#070711", minHeight: "100vh", color: "#fff" }}>
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "100px 24px 60px" }}>
        ...
        {/* Sync Toast / Banner */}
        {syncStatusMessage && (
          <div style={{ background: "rgba(139,92,246,0.15)", border: "1px solid #8b5cf6", borderRadius: "12px", padding: "12px 20px", marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "center", animation: "fadeIn 0.3s ease" }}>
            <span style={{ color: "#c4b5fd", fontWeight: 700, fontSize: "0.9rem" }}>{syncStatusMessage}</span>
          </div>
        )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "32px", alignItems: "center", marginBottom: "48px", background: "rgba(255,255,255,0.02)", padding: "40px", borderRadius: "32px", border: "1px solid rgba(255,255,255,0.05)" }}>
          ...
          <div style={{ flex: 1, minWidth: "250px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px", marginBottom: "4px" }}>
              <div style={{ fontSize: "0.85rem", fontWeight: 800, color: "#8b5cf6", textTransform: "uppercase", letterSpacing: "1px" }}>LEVEL {level}</div>
              {user && (
                <button
                  onClick={syncNow}
                  disabled={isSyncing}
                ...
                >
                  <span style={{ display: "inline-block", transform: isSyncing ? "rotate(360deg)" : "none", transition: isSyncing ? "transform 0.8s linear" : "none" }}>
                    🔄
                  </span>
